  ...
  return (
    <ul className="flex flex-col gap-1.5 pl-1">
      {items.map((item, index) => (
        ...
        >
          {variant === "check" ? (
            <Icon name="check" className="text-tertiary mt-1.5 size-3 shrink-0" />
          ) : (
            <span className="text-tertiary/80 mt-2 inline-block size-1 shrink-0 rounded-full bg-current" />
          )}
          <span>{item}</span>
        </li>
      ))}
...
export const LegalNote = ({ children }: LegalNoteProps) => {
  return (
    <aside className="bg-muted/30 border-border/60 my-2 flex items-start gap-3 rounded-lg border px-4 py-3 text-[13px] leading-[1.65] text-pretty">
